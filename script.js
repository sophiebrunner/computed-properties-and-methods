Vue.createApp({
  ...
  computed: {
    filteredColumnNames() {
      const arrayWithKeys = Object.keys(this.books[0]);
      const keysForFilter = [
        "title",
        "isbn",
        "author",
      ...
      const requiredKeys = arrayWithKeys.filter((key) => {
        return keysForFilter.includes(key);
      });

      return requiredKeys;
    },
    booksFromOReilyMedia() {
  ...
  methods: {
    calculateDiscountedPrice(price, discount) {
      ...
      const priceAfterDiscount = (priceOnly / 100) * discount;
      return "$" + (priceOnly - priceAfterDiscount).toFixed(2);
    },
  },
}).mount("#app");
